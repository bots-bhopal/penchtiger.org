...
class FrontendController extends Controller
{
    public function notification_page()
    {
        ...
        // }])->get();
        $notices_tenders = SubCategory::with(['links' => function ($q) {
            $q->orderBy('updated_at', 'DESC');
        }])->where('parent_id', 1)->get();
        return view('frontend.pages.notification.notifications')->with(['notices_tenders' => $notices_tenders]);
    }

    public function link_page()
    {
        $documents_links = SubCategory::with(['links' => function ($q) {
            $q->orderBy('updated_at', 'DESC');
        }])->where('parent_id', 2)->get();
        return view('frontend.pages.link.links')->with(['documents_links' => $documents_links]);
    }

    public function tcp_page()
    {
        $tcp_informations = SubCategory::with(['links' => function ($q) {
            $q->orderBy('updated_at', 'DESC');
        }])->where('parent_id', 3)->get();
        return view('frontend.pages.touristinfo.touristinfos')->with(['tcp_informations' => $tcp_informations]);
    }

    public function fetch_tender()
    {
        $tenders = Tender::orderBy('updated_at', 'DESC')->get();
        return view('frontend.pages.link.tender-single')->with(['tenders' => $tenders]);
    }

    public function fetch_document()
    {
        $documents = Document::orderBy('updated_at', 'DESC')->get();
        return view('frontend.pages.link.document-single')->with(['documents' => $documents]);
    }

    public function download($filename)
    {
        $file_path = public_path('storage/important_documents/') . $filename;
    ...
    public function downloadTender($filename)
    {
    ...
    {
        $file_path = public_path('storage/other_documents/') . $filename;
        if (file_exists($file_path)) {
            // Load PDF File
            return response()->file($file_path);
        } else {
    ...
    public function index()
    {
        ...
        $all_news = News::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'DESC')->get();
        $all_tenders = Tender::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'DESC')->get();
        $all_documents = Document::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'DESC')->get();
        $all_gallery_images = ImageGallery::all();
        // $all_links = Category::with('links')->get();
        ...
        $blade_data = [
            ...
            'notices_tenders' => $notices_tenders,
            'documents_links' => $documents_links,
            'tcp_informations' => $tcp_informations,
        ];
    ...
    public function home_page_change($id)
    {
        ...
        $all_work = Works::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->take(get_static_option('home_page_01_case_study_items'))->get();
        $all_blog = Blog::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->take(6)->get();
        $all_news = News::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->take(6)->get();
        $all_contact_info = ContactInfoItem::where(['lang' => $lang])->orderBy('id', 'desc')->get();
        $all_service_category = ServiceCategory::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->take(get_static_option('home_page_01_service_area_items'))->get();
        $all_contain_cat = $all_work->map(function ($index) {
            return $index->categories_id;
    ...
    public function news_page()
    {
        $default_lang = Language::where('default', 1)->first();
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
        $all_news = News::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->paginate(get_static_option('news_page_item'));
    ...
    public function news_single_page($slug)
    {
        ...
        return view('frontend.pages.news.news-single')->with([
            'news' => $news
        ]);
    }
    ...
    public function tender_page()
    {
        $default_lang = Language::where('default', 1)->first();
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
        $all_tenders = Tender::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->paginate(get_static_option('tender_page_item'));
        return view('frontend.pages.tender.tenders')->with([
            'all_tenders' => $all_tenders
        ]);
    }

    public function tender_single_page($slug)
    {
        ...
        return view('frontend.pages.tender.tender-single')->with([
            'tender' => $tender
        ]);
    }
    ...
    public function document_page()
    {
        $default_lang = Language::where('default', 1)->first();
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
        $all_documents = Document::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->paginate(get_static_option('document_page_item'));
        return view('frontend.pages.document.documents')->with([
            'all_documents' => $all_documents
        ]);
    }

    public function document_single_page($slug)
    {
        ...
        return view('frontend.pages.document.document-single')->with([
            'document' => $document
        ]);
    }
