...
class IndustryHomePageController extends Controller
{
    ...
    {
        $this->validate($request, [
            'home_page_07_estimate_area_form_email' => 'nullable|email',
        ]);

        foreach ($this->languages as $lang) {
            $this->validate($request, [
                'home_page_07_' . $lang->slug . '_estimate_area_form_button_text' => 'nullable|string',
                'home_page_07_' . $lang->slug . '_estimate_area_form_title' => 'nullable|string',
                'home_page_07_' . $lang->slug . '_estimate_area_title' => 'nullable|string',
            ]);
            $fields_list = [
                'home_page_07_' . $lang->slug . '_estimate_area_form_button_text',
                'home_page_07_' . $lang->slug . '_estimate_area_form_title',
                'home_page_07_' . $lang->slug . '_estimate_area_title',
            ];
        ...
        $fields = [
            'home_page_07_estimate_area_form_email',
        ];
    ...
    public function update_news_area(Request $request)
    {
        ...
        foreach ($all_language as $lang) {
            $this->validate($request, [
                ...
                'industry_news_area_section_' . $lang->slug . '_btn_text' => 'nullable|string',
            ]);
            ...
            $all_fields = [
                ...
                'industry_news_area_section_' . $lang->slug . '_btn_text',
            ];
            foreach ($all_fields as $field) {
                update_static_option($field, $request->$field);
            }
    ...
    {
        $all_languages = Language::all();
        return view($this->industry_home_base_view_path . 'blog-area')->with(['all_languages' => $all_languages]);
    }

    public function update_blog_area(Request $request)
    {
        $all_language = Language::all();
        foreach ($all_language as $lang) {
            $this->validate($request, [
                'industry_blog_area_section_' . $lang->slug . '_subtitle' => 'nullable|string',
                'industry_blog_area_section_' . $lang->slug . '_title' => 'nullable|string',
                'industry_blog_area_section_' . $lang->slug . '_btn_text' => 'nullable|string',
            ]);

            //save repeater values
            $all_fields = [
                'industry_blog_area_section_' . $lang->slug . '_subtitle',
                'industry_blog_area_section_' . $lang->slug . '_title',
                'industry_blog_area_section_' . $lang->slug . '_btn_text',
            ];
            foreach ($all_fields as $field) {
                update_static_option($field, $request->$field);
            }
