...
<head>
    ...
    <style>
        :root {
            ...
            @php $heading_font_family = !empty(get_static_option('heading_font')) ? get_static_option('heading_font_family') :  get_static_option('body_font_family') @endphp --heading-font: "{{ $heading_font_family }}", sans-serif;
    ...
    @endif
    @if (request()->is('news/*') || request()->is('blog/*') || request()->is('work/*') || request()->is('service/*'))
        @yield('og-meta')
        <title>@yield('site-title')</title>
    @elseif(request()->is('about') || request()->is('service') || request()->is('work') || request()->is('team') || request()->is('faq') || request()->is('news') || request()->is('blog') || request()->is('contact') || request()->is('p/*') || request()->is('blog/*') || request()->is('news/*') || request()->is('services/*'))
...
<body>
    ...
    <div class="maintenance-page-content-area" {!! render_background_image_markup_by_attachment_id(get_static_option('maintain_page_background_image')) !!}>
        <div class="container">
                    ...
                    <div class="maintenance-page-inner-content">
                        <div class="page-content-wrap">
                            ...
                            <h2 class="maintain-title">
                                {{ get_static_option('maintain_page_' . $user_select_lang_slug . '_title') }}</h2>
                            <p>{{ get_static_option('maintain_page_' . $user_select_lang_slug . '_description') }}
                            </p>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.newsletter-form-wrap .submit-btn', function(e) {
                ...
                var newsCont = $(
                    '.newsletter-widget .form-message-show,.newsletter-form-wrap .form-message-show')
                ...
                $.ajax({
                    ...
                    error: function(data) {
                        ...
                        newsCont.html('<div class="alert alert-danger">' + errors.email[0] +
                            '</div>');
                    }
                });
            });
