...
@section('content')
    @php
    // $all_permission_list = ['Video Gallery', 'Admin Manage', 'About Page Manage', 'Users Manage', 'Quote Manage', 'Newsletter Manage', 'Package Orders Manage', 'All Payment Logs', 'Pages Manage', 'Menus Manage', 'Widgets Manage', 'Popup Builder', 'Form Builder', 'Blogs Manage', 'News Manage', 'Job Post Manage', 'Events Manage', 'Products Manage', 'Donations Manage', 'Knowledgebase', 'Home Variant', 'Topbar Settings', 'Home Page Manage', 'Contact Page Manage', 'Feedback Page Manage', 'Services', 'Case Study', 'Gallery Page', '404 Page Manage', 'Faq', 'Brand Logos', 'Price Plan', 'Team Members', 'Testimonial', 'Counterup', 'General Settings', 'Languages', 'Courses Manage', 'Appointment Manage', 'Support Tickets', 'Email Templates'];
    $all_permission_list = ['Admin Manage', 'About Page Manage', 'Users Manage', 'Pages Manage', 'Menus Manage', 'Widgets Manage', 'Blogs Manage', 'News Manage', 'Tender Manage', 'Documents Manage', 'Link Manage', 'Topbar Settings', 'Home Page Manage', 'Contact Page Manage', 'Feedback Page Manage', 'Services', 'Gallery Page', '404 Page Manage', 'General Settings', 'Languages', 'Email Templates'];
    @endphp
                            ...
                            <div class="form-group">
                                ...
                                <select name="permission[]" multiple id="permission" class="form-control nice-select wide">
                                    @foreach ($all_permission_list as $per)
                                        <option value="{{ strtolower(str_replace(' ', '_', $per)) }}">
                                            {{ $per }}
                                        </option>
                                    @endforeach
                                </select>
                                <div class="info-text">
                                    {{ __('assign permission to role, which page can seen by the this role') }}</div>
                            </div>
                            <button type="submit"
                                class="btn btn-primary mt-4 pr-4 pl-4">{{ __('Add New Role') }}</button>
                        </form>
                    </div>
                </div>
            </div>
