...
Route::group(['middleware' => ['setlang:frontend', 'globalVariable', 'maintains_mode']], function () {
    ...
    ------------------------------------------ */
    Route::get('/download/{filename}', 'FrontendController@download')->name('user.download');
    Route::get('/tender-download/{filename}', 'FrontendController@downloadTender')->name('user.tender.download');
    Route::get('/document-download/{filename}', 'FrontendController@downloadDocument')->name('user.document.download');
...
Route::group(['middleware' => ['setlang:frontend', 'globalVariable', 'maintains_mode', 'HtmlMinifier']], function () {
    ...
    $news_page_slug = get_static_option('news_page_slug') ?? 'news';
    $tender_page_slug = get_static_option('tender_page_slug') ?? 'tender';
    $document_page_slug = get_static_option('document_page_slug') ?? 'document';
    $notification_page_slug = get_static_option('notification_page_slug') ?? 'notifications';
    $link_page_slug = get_static_option('link_page_slug') ?? 'links';
    $tcp_page_slug = get_static_option('tcp_page_slug') ?? 'tcps';
    ...
    ---------------------------------------*/
    Route::get($news_page_slug, 'FrontendController@news_page')->name('frontend.news');
    Route::get($news_page_slug . '/{slug}', 'FrontendController@news_single_page')->name('frontend.news.single');

    /*--------------------------------------
        FRONTEND: TENDERS ROUTES
    ---------------------------------------*/
    Route::get($tender_page_slug, 'FrontendController@tender_page')->name('frontend.tender');
    Route::get($tender_page_slug . '/{slug}', 'FrontendController@tender_single_page')->name('frontend.tender.single');

    /*--------------------------------------
        FRONTEND: DOCUMENTS ROUTES
    ---------------------------------------*/
    Route::get($document_page_slug, 'FrontendController@document_page')->name('frontend.document');
    Route::get($document_page_slug . '/{slug}', 'FrontendController@document_single_page')->name('frontend.document.single');

    /*--------------------------------------
        FRONTEND: NOTIFICATION ROUTES
    ---------------------------------------*/
    Route::get($notification_page_slug, 'FrontendController@notification_page')->name('frontend.notifications');

    /*--------------------------------------
        FRONTEND: LINKS ROUTES
    ---------------------------------------*/
    Route::get($link_page_slug, 'FrontendController@link_page')->name('frontend.links');

    /*--------------------------------------
        FRONTEND: TCP ROUTES
    ---------------------------------------*/
    Route::get($tcp_page_slug, 'FrontendController@tcp_page')->name('frontend.tcps');
...
Route::prefix('admin-home')->middleware(['setlang:backend'])->group(function () {
    ...
    Route::middleware(['adminPermissionCheck:Home Page Manage'])->group(function () {
       ...
       -----------------------------------------*/
        Route::group(['prefix' => 'home-07'], function () {
            ...
            Route::get('/estimate-area', 'IndustryHomePageController@estimate_area')->name('admin.home07.estimate');
            Route::post('/estimate-area', 'IndustryHomePageController@update_estimate_area');
            ...
            Route::get('/blog-area', 'IndustryHomePageController@blog_area')->name('admin.home07.blog.area');
            Route::post('/blog-area', 'IndustryHomePageController@update_blog_area');
        });
 ...
 ==============================================*/
    ...
    Route::prefix('news')->middleware(['adminPermissionCheck:News Manage'])->group(function () {
        ...
        Route::get('/new', 'NewsController@new_news')->name('admin.news.new');
        Route::post('/new', 'NewsController@store_new_news');
        Route::post('/clone', 'NewsController@clone_news')->name('admin.news.clone');
        Route::get('/edit/{id}', 'NewsController@edit_news')->name('admin.news.edit');
        Route::post('/update/{id}', 'NewsController@update_news')->name('admin.news.update');
        Route::post('/delete/{id}', 'NewsController@delete_news')->name('admin.news.delete');
        Route::post('/bulk-action', 'NewsController@bulk_action')->name('admin.news.bulk.action');
        Route::post('/slug-check', 'NewsController@slug_check')->name('admin.news.slug.check');

        /*-------------------------
           NEWS PAGE SETTINGS ROUTES
        --------------------------*/
        Route::get('/page-settings', 'NewsController@news_page_settings')->name('admin.news.page.settings');
        Route::post('/page-settings', 'NewsController@update_news_page_settings');
    });

    /*==============================================
       LINKS
    ==============================================*/
    Route::prefix('link')->middleware(['adminPermissionCheck:Link Manage'])->group(function () {
         /*-------------------------
           LINK PAGE SETTINGS ROUTES
        --------------------------*/
        Route::get('/page-settings', 'LinkController@links_page_settings')->name('admin.links.page.settings');
        Route::post('/page-settings', 'LinkController@update_links_page_settings');

         /*-------------------------
          LINK CATEGORY ROUTES
        --------------------------*/
        // Route::get('/category-check-slug', 'LinkCategoryController@checkSlug')->name('category.slug.check');
        // Route::get('/category', 'LinkCategoryController@index')->name('admin.categories');
        // Route::get('/new-category', 'LinkCategoryController@create')->name('admin.category.new');
        // Route::post('/new-category', 'LinkCategoryController@store')->name('admin.category.store');
        // Route::get('/{id}/edit/category', 'LinkCategoryController@edit')->name('admin.category.edit');
        // Route::match(['get', 'post'], '/{id}/update/category', 'LinkCategoryController@update')->name('admin.category.update');
        // Route::match(['get', 'delete'], '/{id}/category', 'LinkCategoryController@destroy')->name('admin.category.destroy');

        /*-------------------------
          LINK SUB CATEGORY ROUTES
        --------------------------*/
        Route::get('/subcategory-check-slug', 'SubCategoryController@checkSlug')->name('subcategory.slug.check');
        Route::get('/subcategory', 'SubCategoryController@index')->name('admin.subcategories');
        Route::get('/new-subcategory', 'SubCategoryController@create')->name('admin.subcategory.new');
        Route::post('/new-subcategory', 'SubCategoryController@store')->name('admin.subcategory.store');
        Route::get('/{id}/edit/subcategory', 'SubCategoryController@edit')->name('admin.subcategory.edit');
        Route::match(['get', 'post'], '/{id}/update/subcategory', 'SubCategoryController@update')->name('admin.subcategory.update');
        Route::match(['get', 'delete'], '/{id}/subcategory', 'SubCategoryController@destroy')->name('admin.subcategory.destroy');

        /*-------------------------
          LINK ROUTES
        --------------------------*/
        Route::get('/', 'LinkController@index')->name('admin.links');
        Route::get('/new', 'LinkController@create')->name('admin.link.new');
        Route::post('/new', 'LinkController@store')->name('admin.link.store');
        Route::get('/{id}/edit', 'LinkController@edit')->name('admin.link.edit');
        Route::match(['get', 'post'], '/{id}/update', 'LinkController@update')->name('admin.link.update');
        Route::match(['get', 'delete'], '/{id}', 'LinkController@destroy')->name('admin.link.destroy');    
    });

    /*==============================================
       TENDER
    ==============================================*/
    Route::prefix('tender')->middleware(['adminPermissionCheck:Tenders Manage'])->group(function () {
        /*-------------------------
            TENDER PAGE SETTINGS ROUTES
        --------------------------*/
        Route::get('/page-settings', 'TenderController@tender_page_settings')->name('admin.tender.page.settings');
        Route::post('/page-settings', 'TenderController@update_tender_page_settings');

        /*-------------------------
          TENDER ROUTES
        --------------------------*/
        Route::get('/check-slug', 'TenderController@checkSlug')->name('slug.check');
        Route::post('/bulk-action', 'TenderController@bulk_action')->name('admin.tender.bulk.action');
        Route::get('/', 'TenderController@index')->name('admin.tender');
        Route::get('/new', 'TenderController@create')->name('admin.tender.new');
        Route::post('/new', 'TenderController@store')->name('admin.tender.store');
        Route::get('/{id}/edit', 'TenderController@edit')->name('admin.tender.edit');
        Route::match(['get', 'post'], '/{id}/update', 'TenderController@update')->name('admin.tender.update');
        Route::match(['get', 'post'], '/{id}', 'TenderController@destroy')->name('admin.tender.destroy');
    });
    ...
    /*==============================================
       DOCUMENT
    ==============================================*/
    Route::prefix('document')->middleware(['adminPermissionCheck:Documents Manage'])->group(function () {
        /*-------------------------
            DOCUMENT PAGE SETTINGS ROUTES
        --------------------------*/
        Route::get('/page-settings', 'DocumentController@document_page_settings')->name('admin.document.page.settings');
        Route::post('/page-settings', 'DocumentController@update_document_page_settings');

        /*-------------------------
          DOCUMENT ROUTES
        --------------------------*/
        Route::get('/check-slug', 'DocumentController@checkSlug')->name('slug.check');
        Route::post('/bulk-action', 'DocumentController@bulk_action')->name('admin.document.bulk.action');
        Route::get('/', 'DocumentController@index')->name('admin.document');
        Route::get('/new', 'DocumentController@create')->name('admin.document.new');
        Route::post('/new', 'DocumentController@store')->name('admin.document.store');
        Route::get('/{id}/edit', 'DocumentController@edit')->name('admin.document.edit');
        Route::match(['get', 'post'], '/{id}/update', 'DocumentController@update')->name('admin.document.update');
        Route::match(['get', 'post'], '/{id}', 'DocumentController@destroy')->name('admin.document.destroy');
    });
